...
var ErrUknownPacket = errors.New("unknown packet type")
...
func (d *Dealer) Decode(p *packet.Packet) (v interface{}, err error) {
	pType, ok := d.GetHolderType(p)
	if !ok {
		return nil, ErrUknownPacket
	}

	inst := reflect.New(pType).Elem()
...
func (d *Dealer) GetHolderType(p *packet.Packet) (reflect.Type, bool) {
	d.smu.RLock()
	defer d.smu.RUnlock()
